...
class EditorView(EventPermissionRequiredMixin, ChartContainingView, TemplateView):
    template_name = 'pretix_shaticketoutputpdf/index.html'
    ...
    def post(self, request, *args, **kwargs):
        if "background" in request.FILES:
            ...
            return JsonResponse({
                "status": "ok",
                "id": c.id,
                "url": reverse('plugins:pretix_shaticketoutputpdf:shapdf', kwargs={
        ...
        if "preview" in request.POST:
            ...
            if cf:
                fexisting = request.event.settings.get('shaticketoutput_pdf_layout', as_type=File)
                if fexisting:
                    try:
                ...
                fname = '%s-%s/%s/%s.%s.%s' % (
                    'event', 'settings', self.request.event.pk, 'shaticketoutput_pdf_layout', nonce, 'pdf'
                )
                newname = default_storage.save(fname, cf.file)
                request.event.settings.set('shaticketoutput_pdf_background', 'file://' + newname)

            request.event.settings.set('shaticketoutput_pdf_layout', request.POST.get("data"))
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx['layout'] = json.dumps(
            self.request.event.settings.get('shaticketoutput_pdf_layout', as_type=list)
...
class FontsCSSView(TemplateView):
    ...
    template_name = 'pretix_shaticketoutputpdf/webfonts.css'
